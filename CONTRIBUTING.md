...
- **Cobertura**: `--cov=core --cov=setup --cov=tui` en CI; umbral actual `--cov-fail-under=48` (subida gradual 40 → 45 → 48; próximos PRs deberían apuntar a 55+ cubriendo `tui/app.py` Textual god class, `core/sdrplay_forensics.py` parsers, `setup/windows_installers.py` download/installer).
