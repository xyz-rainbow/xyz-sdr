...
# Patching _is_windows (instead of os.name) keeps these tests portable across
# runners: on Windows, monkeypatching os.name='posix' trips a pytest internal
# Path comparison (PosixPath vs WindowsPath) that crashes the whole session.
def test_wer_status_non_windows(monkeypatch):
    monkeypatch.setattr("core.sdrplay_wer._is_windows", lambda: False)
    status = wer_status()
